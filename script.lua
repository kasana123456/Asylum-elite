--[[ 
    ASYLUM ELITE V11.8 (Modified: Team Check Added)
]]
...
getgenv().Config = {
    ...
    WallCheck = true,
    TeamCheck = true, -- NEW: Team Check Setting
    ShowFOV = true,
    FOVRadius = 150,
    Smoothness = 0.15,
    HitboxEnabled = false,
...
ScreenGui.Name = "AsylumV11_8"; ScreenGui.ResetOnSpawn = false
...
--// UI COMPONENTS (Dropdown, Toggle, Slider)
local function AddDropdown(parent, txt, options, key)
    ...
    local btn = Instance.new("TextButton", f); btn.Size = UDim2.new(1, 0, 1, 0); btn.BackgroundColor3 = Color3.fromRGB(35, 35, 50); btn.Text = txt .. ": " .. getgenv().Config[key]; btn.TextColor3 = Color3.new(1,1,1); btn.Font = "GothamSemibold"; btn.TextSize = 16; Instance.new("UICorner", btn)
    ...
    for _, opt in pairs(options) do
        local o = Instance.new("TextButton", drop); o.Size = UDim2.new(1, 0, 0, 40); o.BackgroundTransparency = 1; o.Text = opt; o.TextColor3 = Color3.new(0.8, 0.8, 0.8); o.Font = "Gotham"; o.TextSize = 16; o.ZIndex = 21
        o.MouseButton1Click:Connect(function()
            getgenv().Config[key] = opt; btn.Text = txt .. ": " .. opt; drop.Visible = false
        end)
    end
end
...
AddToggle(Tabs.Aim.Frame, "Team Check", "TeamCheck") -- Added to UI
...
FOVCircle.Thickness = 1.5; FOVCircle.Color = Color3.fromRGB(0, 150, 255)
    ...
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("Model") and v:FindFirstChildOfClass("Humanoid") and v ~= LP.Character then
            ...
            if not isTeammate then
                local validMode = (mode == "All") or (mode == "Players" and targetPlayer) or (mode == "NPCs" and not targetPlayer)
...
local oldI; oldI = hookmetamethod(game, "__index", function(self, idx)
    if not checkcaller() and (getgenv().Config.Method1_Silent or getgenv().Config.Method2_Silent) and LockedTarget and self == Mouse and (idx == "Hit" or idx == "Target") then
